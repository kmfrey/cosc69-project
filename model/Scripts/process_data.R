...
#   speed.
saccades_detection <- function(samples, lambda, smooth_saccades) {
    library("zoom")
    ...
    # computation of velocity thresholds for the detection algorithm was
    ...
    # Detection thresholds were computed independently for horizontal gx
    # and vertical gy components
    radiusx <- msdx * lambda
    radiusy <- msdy * lambda
...
fixation_webgazerdata_preprocess <- function(input_filename) { # nolint
    df_webgazerdata <- read.csv2(input_filename, sep = ",")

    # Data Preprocessing
    df_webgazerdata <- df_webgazerdata[!duplicated(df_webgazerdata), ]
    df_webgazerdata$Timestamp_utc <-
     strptime(df_webgazerdata$Timestamp_utc, "%Y-%m-%dT%H:%M:%OSZ")
    # op <- options(digits.secs = 3)
    df_webgazerdata$GazeX_b_px <-
     as.numeric(as.character(df_webgazerdata$GazeX_b_px))
    df_webgazerdata$GazeY_b_px <-
     as.numeric(as.character(df_webgazerdata$GazeY_b_px))
    df_webgazerdata$Video_length <-
     as.numeric(as.character(df_webgazerdata$Video_length))
    df_webgazerdata$Video_time <-
     as.numeric(as.character(df_webgazerdata$Video_time))
    df_webgazerdata$GazeX_s_px <-
     as.numeric(as.character(df_webgazerdata$GazeX_s_px))
    df_webgazerdata$GazeY_s_px <-
     as.numeric(as.character(df_webgazerdata$GazeY_s_px))

    # Calculate trials
    temp_list_videolength <- unique(df_webgazerdata$Video_length)
    df_webgazerdata$trial <- 1
    for (i in seq_len(length(temp_list_videolength))) {
        df_webgazerdata[which(df_webgazerdata$Video_length == temp_list_videolength[i]), "trial"] <- i
    }

    # Calculate time
    df_webgazerdata$time <-
     as.numeric(difftime(df_webgazerdata$Timestamp_utc, min(df_webgazerdata$Timestamp_utc))) * 1000
...
fixation_gazedata_execute <- function(df_webgazerdata, lambda) {
    ...
    df_webgazerdata <-
     saccades_detection(df_webgazerdata, lambda = lambda, smooth_saccades = T)

    return(df_webgazerdata)
}

# In our method, we use method to detect saccades.
# Since the detection requency is about 5Hz, which means that the timespan
# between two timestamp is about 200ms.
# However, based on previous work(??), the fixation is about 100 ms to 800 ms,
# which means that we can not detect fixation accurately

fixation_gazedata_postprocess <- function(df_webgazerdata) {
    # Curently, we measure whether the movement bewteen previous eye gaze point
    # and current eye gaze point is a saccade.
    # Then we calculate FixationIndex, FixationPointX (MCSpx),
    # FixationPointY (MCSpx), GazeEventDuration, AbsoluteSaccadicDirection
    ...
    temp_trial <- 1
    for (i in seq_len(nrow(df_webgazerdata))) {
        if (df_webgazerdata[i, "saccade"] == FALSE) {
            # FixationIndex
            # Flase means it is the same fixation from the previous point to 
            # the current point
    ...
    for (temp_index in list_fixationindex) {
        df_webgazerdata[which(df_webgazerdata$FixationIndex == temp_index), "FixationPointX (MCSpx)"] <-
         mean(df_webgazerdata[which(df_webgazerdata$FixationIndex == temp_index), "x"])
        df_webgazerdata[which(df_webgazerdata$FixationIndex == temp_index), "FixationPointY (MCSpx)"] <-
         mean(df_webgazerdata[which(df_webgazerdata$FixationIndex == temp_index), "y"])
        df_webgazerdata[which(df_webgazerdata$FixationIndex == temp_index), "GazeEventDuration"] <-
         max(df_webgazerdata[which(df_webgazerdata$FixationIndex == temp_index), "time"]) - min(df_webgazerdata[which(df_webgazerdata$FixationIndex == temp_index), "time"])
    }
        ...
        if (i == 1) {
            ...
            if (df_webgazerdata[i, "trial"] == temp_trial) {
                if (df_webgazerdata[i, "GazeEventDuration"] == 0) {
                    df_webgazerdata[i, "GazeEventDuration"] <-
                     df_webgazerdata[i, "time"] - df_webgazerdata[i - 1, "time"]
                }
            } else {
                df_webgazerdata[i, "GazeEventDuration"] <- 0
                temp_trial <- df_webgazerdata[i, "trial"]
    ...
    for (i in seq_len(nrow(df_webgazerdata))) {
        if (i == 1) {
            # Initialize temp_fixation_index_2, temp_AbsoluteSaccadicDirection,
            # temp_FixationPointX, temp_FixationPointY
            ...
            if (df_webgazerdata[i, "FixationIndex"] == temp_fixation_index_2) {
                df_webgazerdata[i, "AbsoluteSaccadicDirection"] <-
                 df_webgazerdata[i - 1, "AbsoluteSaccadicDirection"]
            } else {
                # different trial
                # Initialize temp_fixation_index_2,
                # temp_AbsoluteSaccadicDirection, temp_FixationPointX,
                # temp_FixationPointY
                if (df_webgazerdata[i, "trial"] != df_webgazerdata[i, "trial"]) {
                    ...
                    temp_AbsoluteSaccadicDirection <- 0
                    temp_FixationPointX <-
                     df_webgazerdata[i, "FixationPointX (MCSpx)"]
                    temp_FixationPointY <-
                     df_webgazerdata[i, "FixationPointY (MCSpx)"]
                } else {
                    # same trial, different fixation
                    # set
                    temp_fixation_index_2 <- df_webgazerdata[i, "FixationIndex"]
                    temp_AbsoluteSaccadicDirection <-
                     rad2deg(atan((df_webgazerdata[i, "FixationPointY (MCSpx)"] - temp_FixationPointY) / (df_webgazerdata[i, "FixationPointX (MCSpx)"] - temp_FixationPointX)))
                    temp_FixationPointX <-
                     df_webgazerdata[i, "FixationPointX (MCSpx)"]
                    temp_FixationPointY <-
                     df_webgazerdata[i, "FixationPointY (MCSpx)"]
    ...
    require(zoo)
    df_webgazerdata <- transform(df_webgazerdata, AbsoluteSaccadicDirection = na.locf(AbsoluteSaccadicDirection)) # nolint
...
fixation_webgazerdata_trans <- function(input_folderpath, output_folderpath) {
    ...
    for (filename in list_filename) {
        ...
        df_webgazerdata <-
         fixation_webgazerdata_preprocess(filepath_preprocessed)
        df_webgazerdata <-
         fixation_gazedata_execute(df_webgazerdata, lambda = 1)
        ...
        write.csv(x = df_webgazerdata, file = filepath_output, sep = ",")
    }
}

fixation_webgazer_for_mw <- function(input_filepath, output_filepath) {
    library(tools)
    if (tools::file_ext(input_filepath) == "csv") {
        # Generate fixations and saccades
        df_webgazerdata <- fixation_webgazerdata_preprocess(input_filepath)
        df_webgazerdata <- fixation_gazedata_execute(df_webgazerdata, lambda=1)
        df_webgazerdata <- fixation_gazedata_postprocess(df_webgazerdata)
        # Write dataframe into a file
        write.csv(x = df_webgazerdata, file = output_filepath)
    }
}